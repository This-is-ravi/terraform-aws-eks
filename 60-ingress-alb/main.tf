#same as web-alb frontend, creating this ingress alb with terraform and then attaching it to K8S cluster
...
resource "aws_lb" "ingress_alb" {
  ...
  enable_deletion_protection = false #its shld be true
...
resource "aws_lb_listener_rule" "frontend" {
  ...
  condition {
    host_header {
      # expense-dev.csvdaws78s.online --> frontend pod
